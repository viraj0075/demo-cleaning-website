          ...
          {servicesData.map((service, idx) => (
            ...
            >
              ...
              <div className="p-6 sm:p-8 grow flex flex-col justify-between space-y-6">
                <div>
                  <h3 className="text-xl sm:text-2xl font-black text-gray-950 mb-3">{service.title}</h3>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
                    {service.bullets.map((bullet, bulletIdx) => (
                      <div key={bulletIdx} className="flex items-center gap-2.5 text-xs sm:text-sm text-gray-700 font-bold">
                ...
                <div className="border-t border-gray-100 pt-5 flex items-center justify-between">
                  <span className="text-xs text-gray-400 font-bold uppercase tracking-wider">Starting at <strong className="text-gray-950 text-base">{service.price}</strong></span>
                  <button
                    onClick={() => navigate(`/quote?plan=${service.title.toLowerCase().replace(/ \/ | & | /g, '-')}`)}
