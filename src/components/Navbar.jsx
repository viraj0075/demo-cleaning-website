            ...
            <div className="p-4 space-y-2 flex flex-col">
              {navLinks.map((link) => (
                <button
                  key={link.id}
                  onClick={() => handleNavClick(link.id)}
                  className="block w-full text-left text-white px-4 py-2.5 rounded-xl text-sm font-bold transition-all hover:bg-white/10"
                >
                  {link.name}
                </button>
              ))}
              <div className="pt-3 border-t border-white/10 mt-1">
                <Link
                  ...
                  className="block w-full text-center py-3 px-6 text-xs font-bold rounded-full bg-lime-400 text-gray-950 shadow-md transition-all font-black"
                >
                  Book Now
                </Link>
              </div>
